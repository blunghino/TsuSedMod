function [SedSize,Bulk,TrenchIDs]=SED_Size_Reader_03(varargin)
...
for i=1:length(unique_tname)
    ...
    SedSize(i).thickness=data(strmatch('Deposit Thickness (cm)',txt(:,2)),SedSize(i).columns);
    SedSize(i).xdistance=data(strmatch('Distance from shore (m)',txt(:,2)),SedSize(i).columns);
    SedSize(i).zdistance=data(strmatch('Elevation (m)',txt(:,2)),SedSize(i).columns);
